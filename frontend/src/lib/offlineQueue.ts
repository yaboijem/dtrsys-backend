...
function getPhotosDir(): Directory {
  if (!photosDir) {
    photosDir = new Directory(Paths.document, STORAGE_KEYS.offlinePhotosDir);
  }
  return photosDir;
}
...
async function persistSelfie(sourceUri: string): Promise<string | null> {
  try {
    const source = new File(sourceUri);
    if (!source.exists) {
      return null;
    }
    const dest = new File(ensurePhotosDir(), `${newUuid()}.jpg`);
    await source.copy(dest);
    return dest.uri;
  } catch (error) {
    console.warn('Failed to persist offline selfie; queueing punch without photo', error);
    return null;
  }
}

function cleanupSelfies(items: OfflinePunch[]): void {
  for (const item of items) {
    if (!item.selfieUri) {
      continue;
    }
    try {
      const file = new File(item.selfieUri);
      if (file.exists) {
        file.delete();
      }
    } catch (error) {
      console.warn('Failed to delete offline selfie copy', error);
    }
  }
}
...
export async function enqueueOfflinePunch(
  ...
  if (selfieUri) {
    const persisted = await persistSelfie(selfieUri);
    if (persisted) {
      entry.selfieUri = persisted;
    }
  }
  queue.push(entry);
...
async function flushBatchWithPhotos(
  ...
  batch.forEach((p, index) => {
    ...
    try {
      const file = new File(p.selfieUri);
      if (file.exists) {
        form.append(`photos[${index}]`, file as unknown as Blob);
      }
    } catch {
