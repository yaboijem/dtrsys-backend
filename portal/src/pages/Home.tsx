  ...
  return (
    <Screen>
      ...
      <div style={{ marginBottom: spacing.md }}>
        ...
        <div style={{ fontSize: fontSize.sm, marginTop: spacing.md, textAlign: 'center', color: colors.muted }}>
          {isOpen
      ...
      {effectivePunches.length > 0 ? (
        <SectionCard title="Today's punches">
          {effectivePunches.map((p) => (
            ...
            >
              <div style={{ flex: 1 }}>
                <div style={{ ...microLabel, color: colors.muted }}>
                  {p.type === 'time_in' ? 'Time in' : 'Time out'}
                </div>
                <div style={{ fontSize: fontSize.lg, fontWeight: '800', marginTop: 2, color: colors.ink }}>{formatTime(p.timestamp)}</div>
              ...
              {p.type === 'time_out' && p.work_minutes !== null ? (
                <div style={{ fontSize: fontSize.lg, fontWeight: '800', marginLeft: spacing.md, color: colors.ink }}>
      ...
      {queue.length > 0 ? (
        <SectionCard title="Offline queue">
          {queue.map((p) => (
            ...
            >
              <div style={{ flex: 1 }}>
                <div style={{ ...microLabel, color: colors.muted }}>{p.type === 'time_in' ? 'Time in' : 'Time out'}</div>
                <div style={{ fontSize: fontSize.lg, fontWeight: '800', marginTop: 2, color: colors.ink }}>{formatTime(p.timestamp)}</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: spacing.xs, marginTop: spacing.xs }}>
          ...
          {flushResult ? (
            <div style={{ marginTop: spacing.md }}>
              <div style={{ fontSize: fontSize.sm, paddingTop: 2, paddingBottom: 2, color: colors.ink }}>
                Synced: {flushResult.synced} · Failed: {flushResult.failed} · Duplicates: {flushResult.duplicates}
              </div>
              {flushResult.faceIssues > 0 ? (
                <div style={{ fontSize: fontSize.sm, paddingTop: 2, paddingBottom: 2, color: colors.dangerText, fontWeight: '600' }}>
